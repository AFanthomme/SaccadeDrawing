'''
Utilities to aggregate data from runs of "full_coverage.py", mostly copy-pasted from aggregate_all_rules.py
'''
...
def do_comparisons():
    ...
    def compare_completion_rate_across_rules():
        ...
        # NOTE: Colors are the same as in all_rules, but the rules they correspond to are different
        fig, ax = plt.subplots(1, 1, figsize=(8, 6))
        for rule, cname in zip(['closest', 'leftward', 'leftward_with_another_color', 'leftward_closest_endpoints', 'closest_from_left'], ['blue', 'orange', 'green', 'pink', 'purple']):
    ...
    def compare_saccades_across_rules():
        ...
        for rule_idx, rule, cname in zip(range(5), ['closest', 'leftward', 'leftward_with_another_color', 'leftward_closest_endpoints', 'closest_from_left'], ['blue', 'orange', 'green', 'pink', 'purple']):
            ...
            if rule in ['leftward', 'leftward_with_another_color', 'closest_from_left']:
                # These are wlways drawn from left to right within one symbol, no need to split
                # Also, we can easily compare the two leftwards, in case network does not figure out it can just merge the two in representation layers
            ...
            elif rule in ['closest', 'leftward_closest_endpoints']:
                # Need to subdivide the closest rule into 2, depending on whether we draw the line in the "usual" direction or not
                # NOTE: same red/blue color scheme for both rules, just because it's the most interpretable "within-plot"
    ...
    def saccade_split_by_seed():
        ...
        axes[0].set_title('Drawing steps (Unimodal rules)')
        axes[0].set_xlabel('X saccade (drawing steps)')
        axes[0].set_ylabel('Y saccade (drawing steps)')
        axes[0].axhline(0, color='black', linestyle='--')
        axes[0].axvline(0, color='black', linestyle='--')
        ...
        axes[1].set_title('Drawing steps (Bimodal rule)')
        ...
        for seed in range(n_seeds):
            ...
            is_unimodal_rule = (np.isin(rules, ['leftward', 'leftward_with_another_color', 'closest_from_left'])) & random_selector_draw
            is_bimodal_rule = (np.isin(rules, ['closest', 'leftward_closest_endpoints'])) & random_selector_draw


            # All unimodal rules
            data = saccades[is_drawing & ~is_unimodal_rule, :2]  
            sns.kdeplot(x=data[:,0], y=data[:,1], ax=axes[0], color=palette[seed], fill=True, levels=5, alpha=0.75)

            # Further split between closest and other rules
            data = saccades[is_drawing & is_bimodal_rule, :2]  
        ...
        axes[0].legend(handles=[plt.Line2D([0], [0], color=palette[seed], lw=4, label=f"Seed {seed}, all unimodal rules") for seed in range(n_seeds)], loc='upper left')
        ...
        axes[0].legend(handles=[plt.Line2D([0], [0], color=palette[seed], lw=4, label=f"Seed {seed}, all bimodal rules") for seed in range(n_seeds)], loc='upper left')
