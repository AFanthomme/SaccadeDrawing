...
def main(args):
    ...
    # Important: seeding
    seed = args.training_params['seed']
    np.random.seed(seed)
    ...
    for update in range(args.training_params['n_updates']):
        ...
        if update % args.training_params['test_every'] == 0:
            ...
            for b_id in range(10):
                ...
                axes[0].plot([start_pos[0], start_pos[0]+target_s[0]], [start_pos[1], start_pos[1]+target_s[1]], marker='+', color='red', label='Target saccade')
                axes[0].plot([start_pos[0], start_pos[0]+pred_s[0]], [start_pos[1], start_pos[1]+pred_s[1]], marker='x', color='green', label='Predicted saccade')
                axes[0].set_title('Peripheral vision system')
                axes[0].legend()

                axes[1].imshow(f_obs.transpose(2, 1, 0), extent=[-1/4, 1/4, -1/4, 1/4], origin='lower')
                axes[1].plot([0., pred_h[0]], [0., pred_h[1]], lw=5, marker='x', color='green', label='Predicted homing')
            ...
            for step in range(0, 20):
                ...
                for env_idx in range(5):
                    ...
                    axes[0].plot([test_envs.positions[env_idx, 0], test_envs.positions[env_idx, 0] + test_action[env_idx, 0]], [test_envs.positions[env_idx, 1], test_envs.positions[env_idx, 1] + test_action[env_idx, 1]], color='gray', label='Total action')
                    ...
                    axes[1].scatter([test_homings[env_idx, 0]], [test_homings[env_idx, 1]], color='m', marker='+', label='Homing submove')
